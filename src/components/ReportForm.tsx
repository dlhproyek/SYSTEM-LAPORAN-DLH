  ...
  const uploadTaskPhotos = async (tasks: any[]) => {
    ...
    for (let i = 0; i < updatedTasks.length; i++) {
      ...
      if (task.photos.zero?.startsWith('data:image')) {
        if (isEditing && initialData?.tasks[i]?.photos.zero) await storageService.deletePhotoByUrl(initialData.tasks[i].photos.zero);
        task.photos.zero = await storageService.uploadPhoto(task.photos.zero, `task_${i}_0`);
      }
      if (task.photos.fifty?.startsWith('data:image')) {
        if (isEditing && initialData?.tasks[i]?.photos.fifty) await storageService.deletePhotoByUrl(initialData.tasks[i].photos.fifty);
      ...
      if (task.photos.hundred?.startsWith('data:image')) {
        if (isEditing && initialData?.tasks[i]?.photos.hundred) await storageService.deletePhotoByUrl(initialData.tasks[i].photos.hundred);
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 max-w-4xl mx-auto pb-20">
        ...
        <div className="flex justify-end gap-4"><Button type="button" variant="outline" onClick={() => navigate(-1)}>Batal</Button><Button type="submit" disabled={isSubmitting || isPimpinan} className={cn("bg-blue-600 hover:bg-blue-700 px-8", isPimpinan && "opacity-50 cursor-not-allowed")}>{isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> {uploadProgress || "Menyimpan..."}</> : "Simpan Laporan"}</Button></div>
      </form>
